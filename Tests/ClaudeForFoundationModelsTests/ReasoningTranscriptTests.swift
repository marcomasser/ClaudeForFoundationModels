...
/// A `LanguageModel` whose executor is a real `ClaudeExecutor` over a canned
/// SSE body, so `LanguageModelSession` exercises the full pipeline offline.
...
struct StubbedExecutor: LanguageModelExecutor {
  ...
  init(configuration: Configuration) throws {
    self.inner = ClaudeExecutor(
      ...
      transport: MockTransport(body: configuration.fixture)
    )
  }
  ...
  func respond(
    ...
    try await inner.respond(
      to: request,
      model: ClaudeLanguageModel(name: .sonnet5, auth: .apiKey("sk-test")),
